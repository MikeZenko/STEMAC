...
const Hero = () => {
  ...
  useEffect(() => {
    setIsVisible(true);
    // Detect Windows to provide emoji fallback rendering
  ...
  // Convert an emoji to a Twemoji-compatible codepoint path (e.g., "1f1f0-1f1f7")
  ...
  const renderFlag = (emoji: string, name: string) => {
    if (!isWindows) return emoji;
    const code = toTwemojiCodePoint(emoji);
                ...
                {countries.map((country, index) => (
                  ...
                  >
                    ...
                    <div className="flag-emoji-container">
                      ...
                      >
                        {renderFlag(country.flag, country.name)}
                      </span>
                    </div>
